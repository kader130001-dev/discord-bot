...
class TicketSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        existing = [ch for ch in guild.text_channels if ch.name.startswith(f"ticket-{member.name.lower()}")]
        if existing:
            return await interaction.response.send_message(
                f"Tu as deja un ticket ouvert : {existing[0].mention}", ephemeral=True
            )

        category = discord.utils.get(guild.categories, name=t["categorie"])
        if not category:
            return await interaction.response.send_message(
                f"La categorie **{t['categorie']}** est introuvable.", ephemeral=True
            )
...
class SettingsView(discord.ui.View):
    ...
    def build_embed(self, config):
        ...
        val = "\n".join([f"**{t['nom']}** — {t['description'][:40]}" for t in types]) if types else "Aucun type configure"
        e.add_field(name=f"Types de tickets ({len(types)})", value=val, inline=False)

        roles = config.get("staff_roles", [])
        val_roles = "\n".join([f"<@&{r}>" for r in roles]) if roles else "Aucun role staff configure"
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if choix == "ajouter":
            ...
            config["types"].append({"nom": nom, "description": description, "emoji": emoji, "categorie": categorie})
