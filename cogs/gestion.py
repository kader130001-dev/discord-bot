...
async def get_log_channel(bot, guild, type_log):
    ...
    if guild_id in data and type_log in data[guild_id]:
        ch = guild.get_channel(data[guild_id][type_log])
        return ch
    return None

def embed_success(titre, fields, moderateur=None):
    e = discord.Embed(title=f"✅  {titre}", color=VERT, timestamp=datetime.utcnow())
...
def embed_error(titre, description):
    e = discord.Embed(title=f"❌  {titre}", description=f"> {description}", color=ROUGE, timestamp=datetime.utcnow())
    e.set_footer(text="⬡ Système de Gestion")
    return e

def embed_warn(titre, fields):
    e = discord.Embed(title=f"⚠️  {titre}", color=ORANGE, timestamp=datetime.utcnow())
    for name, value in fields:
        e.add_field(name=name, value=value, inline=False)
    e.set_footer(text="⬡ Système de Gestion")
    return e
...
def embed_info(titre, fields, thumbnail=None):
    e = discord.Embed(title=f"✦  {titre}", color=get_color(), timestamp=datetime.utcnow())
    for name, value in fields:
        e.add_field(name=name, value=value, inline=False)
    if thumbnail:
...
class HelpView(discord.ui.View):
        ...
        self.pages = [
            ...
            },
            {
                ...
                "fields": [
                    ...
                    ("🔹 +setlogs <type> #salon", "Configure les salons de logs"),
                    ("🔹 Types de logs", "role, ticket, ban, bl"),
                ]
            },
        ]
    ...
    def build_embed(self):
        ...
        e = discord.Embed(title=f"📖  Aide — {page['title']}", color=get_color(), timestamp=datetime.utcnow())
...
class Gestion(commands.Cog):
    ...
    async def ping(self, ctx):
        ...
        e = discord.Embed(title="🏓  Pong !", color=couleur, timestamp=datetime.utcnow())
    ...
    async def setlogs(self, ctx, type_log: str, salon: discord.TextChannel):
        types_valides = ["role", "ticket", "ban", "bl"]
        if type_log not in types_valides:
            await ctx.send(embed=embed_error("Type invalide", f"Types disponibles : `role`, `ticket`, `ban`, `bl`"))
            return
        data = load_logs()
    ...
    async def theme(self, ctx, couleur: str):
        ...
        e = discord.Embed(title="🎨  Thème mis à jour", description=f"La couleur des embeds est maintenant **{couleur}**", color=THEMES[couleur], timestamp=datetime.utcnow())
    ...
    async def clear(self, ctx, nombre: int = 10):
        ...
        msg = await ctx.send(embed=embed_success(
            "Messages supprimés",
            [("🗑️ Quantité", f"`{nombre}` messages"), ("📌 Salon", f"{ctx.channel.mention}")],
            moderateur=ctx.author.name
        ))
    ...
    async def renew(self, ctx):
        ...
        nouveau = await ctx.guild.create_text_channel(name=nom, overwrites=overwrites, category=categorie, position=position)
        msg = await nouveau.send(embed=embed_success("Salon recréé", [("📌 Salon", f"`#{nom}`"), ("👤 Par", f"`{ctx.author.name}`")]))
        await asyncio.sleep(3)
        await msg.delete()
    ...
    async def kick(self, ctx, membre: typing.Union[discord.Member, int], *, raison="Aucune raison"):
        ...
        await ctx.send(embed=embed_success(
            "Membre expulsé",
            [("👤 Membre", f"`{membre.name}` • `{membre.id}`"), ("📋 Raison", f"`{raison}`")],
            moderateur=ctx.author.name
        ))
    ...
    async def ban(self, ctx, membre: typing.Union[discord.Member, int], *, raison="Aucune raison"):
        ...
        log_ch = await get_log_channel(self.bot, ctx.guild, "ban")
        if log_ch:
            await log_ch.send(embed=embed_success(
                "Membre banni",
                [("🔨 Membre", f"`{membre.name}` • `{membre.id}`"), ("📋 Raison", f"`{raison}`")],
                moderateur=ctx.author.name
            ))
                ...
                for entry in bans:
                    if entry.user.id == user.id:
                        await ctx.send(embed=embed_info(
                            "Informations du ban",
                            [("👤 Membre", f"`{entry.user.name}` • `{entry.user.id}`"), ("📋 Raison", f"`{entry.reason or 'Aucune raison'}`")],
    ...
    async def addrole(self, ctx, membre: discord.Member, role: discord.Role):
        ...
        log_ch = await get_log_channel(self.bot, ctx.guild, "role")
        if log_ch:
            await log_ch.send(embed=embed_success(
                "Rôle ajouté",
                [("👤 Membre", f"{membre.mention} • `{membre.id}`"), ("🎭 Rôle", f"{role.mention}")],
                moderateur=ctx.author.name
            ))
        await ctx.send(embed=embed_success(
            "Rôle ajouté",
    ...
    async def userinfo(self, ctx, membre: discord.Member = None):
        ...
        e = discord.Embed(title=f"👤  {membre.name}", color=get_color(), timestamp=datetime.utcnow())
    ...
    async def perms(self, ctx):
        data = load_perms()
        e = discord.Embed(title="🔐  Permissions du serveur", color=get_color(), timestamp=datetime.utcnow())
    ...
    async def setperms(self, ctx, perm: str, *, roles: str):
        ...
        await ctx.send(embed=embed_success(
            ...
            [("🔐 Permission", f"`{perm}`"), ("👥 Rôles", f"{roles}")],
            moderateur=ctx.author.name
        ))
    ...
    async def bl(self, ctx, membre: typing.Union[discord.Member, int], *, raison="Aucune raison"):
        ...
        if guild_id not in data:
            data[guild_id] = {}
        data[guild_id][str(membre.id)] = {"nom": membre.name, "raison": raison, "par": ctx.author.name}
        save_bl(data)
        log_ch = await get_log_channel(self.bot, ctx.guild, "bl")
        if log_ch:
            await log_ch.send(embed=embed_success(
                "Membre blacklisté",
                [("🚫 Membre", f"`{membre.name}` • `{membre.id}`"), ("📋 Raison", f"`{raison}`"), ("⚡ Action", "`Banni du serveur`")],
                moderateur=ctx.author.name
            ))
        await ctx.send(embed=embed_success(
            "Membre blacklisté",
    ...
    async def blinfo(self, ctx, membre: typing.Union[discord.Member, int]):
        ...
        if guild_id in data and membre_id in data[guild_id]:
            ...
            await ctx.send(embed=embed_info(
                ...
                [("🚫 Membre", f"`{info['nom']}` • `{membre_id}`"), ("📋 Raison", f"`{info['raison']}`"), ("👮 Par", f"`{info['par']}`")],
                thumbnail=thumbnail
            ))
        else:
            await ctx.send(embed=embed_error("Introuvable", f"`{membre_nom}` n'est pas dans la blacklist"))
